...
test('grammar.binary.SerializableConverter', () => {
  ...
  should('convert forward and backward undefined correctly', () => {
    const forwardResult = (
      _.converter.convertForward(undefined) as SuccessResult<Uint8Array>
    ).result;
    assert(_.converter.convertBackward(forwardResult)).to.haveProperties({
      result: objectThat<BinaryData<unknown>>().haveProperties({
        data: undefined,
      }),
    });
  });
  ...
  should('convert forward and backward number correctly', () => {
    const forwardResult = (
      _.converter.convertForward(1.234) as SuccessResult<Uint8Array>
    ).result;
    assert(_.converter.convertBackward(forwardResult)).to.haveProperties({
