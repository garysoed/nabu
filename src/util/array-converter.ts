  ...
  return {
    convertBackward(input: readonly T[]): Result<readonly F[]> {
      return input
          ...
          .reduce(
              (result: Result<readonly F[]>, current) => reduceResults(result, current),
              success([]),
          );
    },
    ...
    convertForward(input: readonly F[]): Result<readonly T[]> {
      return input
          ...
          .reduce(
              (result: Result<readonly T[]>, current) => reduceResults(result, current),
              success([]),
          );
    },
  };
...
function reduceResults<T>(acc: Result<readonly T[]>, curr: Result<T>): Result<readonly T[]> {
  if (!acc.success) {
    return acc;
  }
