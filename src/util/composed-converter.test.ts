...
test('util.ComposedConverter', () => {
  test('convertBackward', () => {
    should('convert correctly', () => {
      ...
      assert(converter.convertBackward('abc')).to.haveProperties({
        success: false,
      });
    });
    ...
    should('convert correctly for one converter', () => {
      ...
      assert(converter.convertBackward('ab')).to.haveProperties({
        success: false,
      });
    });
  });
  ...
  test('convertForward', () => {
    should('convert correctly', () => {
      const converter = reverse(
        compose(new PostfixConverter('a'), new PostfixConverter('b')),
      );

      assert(converter.convertForward('cab')).to.haveProperties({result: 'c'});
      assert(converter.convertForward('abc')).to.haveProperties({
        success: false,
      });
    });
    ...
    should('convert correctly for one converter', () => {
      ...
      assert(converter.convertForward('ab')).to.haveProperties({
        success: false,
      });
    });
  });
