...
test('@nabu/src/util/array-converter', () => {
  test('convertBackward', () => {
    should('convert correctly', () => {
      assert(array(EVEN_CONVERTER).convertBackward([1, 3, 5])).to.haveProperties({
        success: true,
    ...
    should('fail if one of the items failed', () => {
      assert(array(EVEN_CONVERTER).convertBackward([1, 4, 5])).to.haveProperties({success: false});
    });
  });
  ...
  test('convertForward', () => {
    ...
    should('fail if one of the items failed', () => {
      assert(array(EVEN_CONVERTER).convertBackward([0, 3, 4])).to.haveProperties({success: false});
    });
  });
});